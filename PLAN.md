...
- Federation stays: each source is a sibling project publishing CSVs; the hub is a read-only join; dedup lives in each scraper.
  **Adapted 2026-09-16** for Phase 5 (local government budgets + general fund
  receipts): those two sources are a third party's repos (not this
  project's own), so "dedup lives in each scraper" has no scraper of this
  project's to apply to — see Phase 5 below for what changed instead.
...
## Phase 5 — Local government budgets and general fund receipts

**Done 2026-09-16.** Two sources from `mattwaite/nebraska-budget-data` and
`mattwaite/nebraska-general-fund-receipts` — MIT-licensed, but **not this
project's own repos**: the professor's, whose course this project grew out
of. This is the first phase where a "sibling repo" is read-only in the
strongest sense — cloned locally the same way as every other source, but
never pushed to, never forked, no CI added there. **Adapts the "Decisions
made" section above:** "federation stays" and "dedup lives in each scraper"
assumed a scraper of this project's own to extend; there isn't one here.
Both source repos are one-shot bulk downloaders with a committed CSV
snapshot already run — nothing to schedule, no workflow added (a first for
this project's phases).

**5.1 Entity source: local government budgets.** Local-government
subdivisions (counties, cities, school districts, fire districts, SIDs,
NRDs, community colleges, airport authorities, etc.) are **government
bodies themselves** — a new kind of entity; everything else in this project
is a vendor/contributor/lobbyist/filer *interacting with* government. Some
(counties, big cities, NPPD, University of Nebraska) already cross-reference
today; most individual subdivisions won't.

Because there is no live site on the professor's repo to cross-fetch from
(the pattern the other four sources use), this project's own build publishes
the itemized data itself instead: new `build/export_budget.py` writes
`d/budget_rows.json`, committed directly (12.5 MB raw CSV in, no sharding
needed — see `docs/SCHEMA.md`'s "Self-published: nebraska-budget-data" for
the full design, including the real entity-identity risk found and fixed
(two genuinely different bodies sharing a name across counties — confirmed
against the live 63,934-row file: exactly two, `"Emerson Hubbard Public
Schools"` and `"Wakefield CRA"`) and the historical
`"Municipalities"`→`"Cities and Villages"` subdivision_type relabel merged
so 530 cities don't each split into two entities).

`ingest/sources.py load_budget_subdivisions()`: `entity_type="organization"`
always (automatically eligible for the org-only auto-accept band, no
resolve/ changes needed). `total_amount` is the most recent fiscal year's
property tax request ONLY, never summed across the up-to-27 years on file —
unlike a one-time contract award, an annual tax request recurs every year,
and summing it would overstate the total by an order of magnitude.
`build/build_entities.py` wires `budgets` into three pairings (contracts,
campaign finance, lobbying — not disclosures or fec, no plausible
counterpart); bit 64 in `build_site.py`'s `SOURCE_BITS` (never 8, reserved
for SoS). `resolve/aliases.yml` gained curated entries for the state's
largest cities (the source's bare `"Lincoln"` vs. contracts/lobbying's
`"City of Lincoln"`); counties need none, since a county's own
`subdivision_name` already matches contracts/lobbying exactly.

**5.2 Non-entity context: General Fund Receipts.** A single statewide
monthly actual-vs-projected tax receipt series — never tied to any entity,
so it never touches `SOURCE_BITS` or the resolution pipeline at all. Inlined
directly into the page as a small, always-visible section near the footer
(not gated behind a per-entity click, since it isn't tied to one) — see
`docs/SCHEMA.md`'s "Self-published (inline)" section.

**Verified against real data:** `entities_in_two_or_more_sources` went from
1,877 to 2,055 (+178, e.g. City of Lincoln, Douglas County now show budget
figures alongside their existing contracts/lobbying records). 25 new tests
(120 → 145) across `tests/test_sources.py`, `tests/test_build_site.py`, and
new `tests/test_export_budget.py` — covering the org-only entity_type, the
most-recent-year-only snapshot rule (never summed, including the edge case
of more than one budgets member for one entity), the two real name
collisions, the Municipalities relabel merge, and the GFR revision-history
pass-through. Confirmed end to end in a real browser: the dossier panel's
itemized budget table and the inline GFR section both render real data
correctly.

Tests: see above. Risks: the professor's repo could change shape without
notice, since it's outside this project's own CI — no schema-shape assertion
added yet, worth adding if this becomes a recurring source of drift. Effort:
~1 day (both sources already collected and licensed; the work was pipeline
integration, not scraping).
